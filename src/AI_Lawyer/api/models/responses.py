...
class DomainAwareQueryResponse(BaseResponse):
    """Response model for domain-aware query endpoint with verification."""
    
    query: str = Field(..., description="Original user query")
    answer: str = Field(..., description="Generated answer from LLM")
    domain_used: str = Field(..., description="Legal domain used for retrieval (e.g., 'bns_criminal_law')")
    domain_confidence: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Confidence score of domain classification (0-1)"
    )
    citations: List[str] = Field(
        default_factory=list,
        description="Extracted and validated citations from answer"
    )
    verified: bool = Field(..., description="Whether answer passed verification")
    confidence_score: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Overall confidence score including verification (0-1)"
    )
    confidence_category: str = Field(
        ...,
        description="Human-readable confidence category (Very High, High, Medium, Low, Very Low)"
    )
    results: List[QueryResult] = Field(
        default_factory=list,
        description="Retrieved context chunks used for answer generation"
    )
    result_count: int = Field(..., description="Number of retrieved results")
    processing_time_seconds: float = Field(
        ...,
        description="Total processing time in seconds"
    )
    fallback_message: Optional[str] = Field(
        None,
        description="Fallback message if verification failed (low confidence)"
    )
    verification_details: Optional[Dict[str, Any]] = Field(
        None,
        description="Detailed verification metrics"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "answer": "Section 303 of the BNS prescribes imprisonment and fine for theft...",
                "domain_used": "bns_criminal_law",
                "domain_confidence": 0.95,
                "citations": ["Section 303 BNS", "Section 304 BNS"],
                "verified": True,
                ...
                "results": [
                    {
                        "success": True,
                        "message": "",
                        "text": "Section 303 BNS - Punishment for theft...",
                        "source": "BNS_2023.pdf",
                        "score": 0.93,
                        "rank": 1,
                        "source_type": "legal_db",
                        "metadata": {"domain": "bns_criminal_law"}
                    }
                ],
                "result_count": 3,
